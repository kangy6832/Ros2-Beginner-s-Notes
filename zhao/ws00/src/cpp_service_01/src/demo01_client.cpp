...
class CppClient : public rclcpp::Node{
    ...
    public:
        ...
        bool connect_server(){
            while(client_->wait_for_service(1s)==false){
                if(rclcpp::ok()==false){
        ...
        auto send_request(int32_t num1, int32_t num2){
...
int main(int argc, char** argv){
    ...
    auto respone = client->send_request(std::stoi(argv[1]), std::stoi(argv[2]));
    if(rclcpp::spin_until_future_complete(client, respone)==rclcpp::FutureReturnCode::SUCCESS){
        RCLCPP_INFO(client->get_logger(), "响应结果：%d", respone.get()->sum);
    }
    rclcpp::shutdown();
    return 0;
}